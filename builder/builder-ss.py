...
class Builder:
    def __init__(self, hit: Hit):
        #TODO: TAKE HHS FROM FUZZLE
        self.HHSPATH = f'/agh/projects/Fuzzle/data/scop95_2.07.psi.hhs/'
    ...
    def seq2pdbaln(self, sequence: str, pdb: Molecule) -> List:
        """ Obtains the resid (positions in the PDB) of the
        aminoacids involved in the fragment.

        :param sequence: str. sequence of the fragment
        :param pdb: Molecule. PDB from where to obtain the residues
        :return: mapping. The the PDB Positions of the fragment.
        A List of tuples of the form: (sequence aminoacid, pdb resid)
        """
        ...
        # Mapping secondary structure to number
        ss = MetricSecondaryStructure(copy_pdb, 'protein and name CA')
        pdb_ss = ss.project(copy_pdb)[0]

        # sequence from the HHS alignment (fragment)
        seq_positions = [(x,) for x in sequence]
        seq_ss = [(x,) for x in sequence]
        ...
        for block in matcher.get_matching_blocks():
            ...
            for pos in range(block.a, block.a + block.size):
                ...
                seq_ss[pos] += (pdb_ss[block.b + i],)
                i += 1
        return seq_positions, seq_ss
    ...
    def _get_pairs(self, aln: HHpredHitAlignment):
        ...
        query_seq2pdb, query_seq2ss = self.seq2pdbaln(aln.query, self.qPDB)
        sbjct_seq2pdb,sbjct_seq2ss = self.seq2pdbaln(aln.subject, self.sPDB)
        ...
        for index, resi in enumerate(aln.columns):
            ...
            if aln.gap_at(index) is False and query_seq2ss[index - 1][1] == sbjct_seq2ss[index - 1][1]:
    ...
    def build_chimeras(self, partial_alignment: bool = False, cutoff_distance: float = 1) -> Dict[str, Chimera]:
        ...
        outcomes = ['Query N-terminal', 'Subject N-terminal', 'Not enough mutations Query N-terminal',
                    'Not enough mutations Subject N-terminal', 'BB clash query N-terminal',
                    'BB clash subject N-terminal']
        ...
        for aln_index, chunk in enumerate(dst):
            ...
            for index in fusion_points:
                ...
                try:
                    ...
                    self.chim_positions['BB clash query N-terminal'].append(xo_query)
                ...
                try:
                    ...
                    self.chim_positions['BB clash subject N-terminal'].append(xo_query)
    ...
    def plot_curves(self):
        """ Plots the distance between the alpha carbons in the structure for the fragment region
        along with the number of backbone clashes of the resulting chimera for each position

        :param dst: np.ndarray: an array containign the distance for each fragment position
        :param bbContacts1: an array containing the number of bb contacts for each fragment position for the
        resulting chimera of combination query-subject
        :param bbContacts2: an array containing the number of bb contacts for each fragment position for t
        resulting chimera of combination subject - query
        ...
        color = [('#FCB711', "X"), ('#CC004C', "x"), ('gray', 'o'), ('gray', "o"), ('black', "."), ('black', ".")]
        colors = dict(zip(resids.keys(), color))

        fig, ax = plt.subplots(figsize=(12, 9))
        ax.plot(residues, dst, '-', color='black', label='distance q-s')
        ax.set_xlabel(f"Residue in the fragment relative to domain {self.qPDB.viewname[:-4]}", fontsize=24)
