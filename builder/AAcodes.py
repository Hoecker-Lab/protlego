'''
Contains the Amino Acid Codes as dictionaries
'''
...
# special2standard = {'MSE': 'MET', 'OMT': 'MET', 'OCS': 'CYS', 'CME': 'MET', 'PCA': 'GLU','DHA':'ALA',
#                     'LA2':'LYS'}
...
one_letter = {
        ...
        'LKC': 'N', 'LLP': 'K', 'LLY': 'K', 'LLZ': 'X', 'LME': 'E', 'LMQ': 'Q',
...
def one_letter_for_correct_residue(res):
    '''Return the Residue Name if there exists a Calpha atom and the alternative location is either ' ', 'A', or '1'
    Input: Bio.PDB.Residue object
    '''
    if res.has_id('CA') and len([ atm for atm in res.get_unpacked_list() if atm.get_id() == 'CA' and atm.get_altloc() in (' ', 'A', '1') ]):
        aa = one_letter.get(res.get_resname(), 'X')
        return ('X', aa)[ len(aa) == 1 ]
    else:
        return 'X'                  
